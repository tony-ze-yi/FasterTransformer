...
class ViTINT8PluginLoader:
    def __init__(self, plugin_path) -> None:
        ...
        self.plg_creator = plg_registry.get_plugin_creator(
            "CustomVisionTransformerINT8Plugin", "1", ""
        )
    ...
    def load_model_config(self, config, args):
        ...
        self.seq_len_ = (
            pow(self.img_size_ // self.patch_size_, 2) + 1
            if self.with_class_token_
            else 0
        )
        self.in_chans_ = 3
        self.int8_mode_ = args.int8_mode
        self.serial_name_ = "ViTINT8Engine_{}_{}_{}_{}_{}_{}_{}_{}_{}".format(
            self.patch_size_,
            self.num_heads_,
            self.layer_num_,
            self.inter_size_,
    ...
    def build_plugin_field_collection(self, weights):
        ...
        self.value_holder = [
            np.array([self.max_batch_]).astype(np.int32),
        ...
        max_batch = trt.PluginField(
            "max_batch", self.value_holder[0], trt.PluginFieldType.INT32
        )
        img_size = trt.PluginField(
            "img_size", self.value_holder[1], trt.PluginFieldType.INT32
        )
        patch_size = trt.PluginField(
            "patch_size", self.value_holder[2], trt.PluginFieldType.INT32
        )
        in_chans = trt.PluginField(
            "in_chans", self.value_holder[3], trt.PluginFieldType.INT32
        )
        embed_dim = trt.PluginField(
            "embed_dim", self.value_holder[4], trt.PluginFieldType.INT32
        )
        num_heads = trt.PluginField(
            "num_heads", self.value_holder[5], trt.PluginFieldType.INT32
        )
        inter_size = trt.PluginField(
            "inter_size", self.value_holder[6], trt.PluginFieldType.INT32
        )
        layer_num = trt.PluginField(
            "layer_num", self.value_holder[7], trt.PluginFieldType.INT32
        )
        int8_mode = trt.PluginField(
            "int8_mode", self.value_holder[8], trt.PluginFieldType.INT32
        )
        with_cls_token = trt.PluginField(
            "with_cls_token", self.value_holder[9], trt.PluginFieldType.INT32
        )
        ...
        for name in weights.keys():
            if name == "transformer.embeddings.cls_token" and (
                not self.with_class_token_
            ):
                continue
            elif (
                name.split(".")[-1] == "amaxList" or name.split(".")[-1] == "h_amaxList"
            ):
                self.value_holder.append(weights[name].cpu().numpy().astype(np.float32))
                part_fc.append(
                    trt.PluginField(
                        name, self.value_holder[-1], trt.PluginFieldType.FLOAT32
                    )
                )
            else:
                ...
                part_fc.append(
                    trt.PluginField(
                        name, self.value_holder[-1], trt.PluginFieldType.FLOAT16
                    )
                )
    ...
    def build_network(self, weights):
        ...
        with trt.Builder(self.logger_) as builder, builder.create_network(
            explicit_batch_flag
            ...
            input_tensor = network.add_input(
                name="input_img",
                dtype=trt.float16,
                shape=(-1, self.in_chans_, self.img_size_, self.img_size_),
            )
