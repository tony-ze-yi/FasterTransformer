...
def setup(args):
    ...
    if args.posttrain:
        model = VisionTransformerINT8(
            config, args.img_size, zero_head=False, num_classes=5
        )
        model_ckpt = torch.load(args.pretrained_dir, map_location="cpu")
        model.load_state_dict(
            model_ckpt["model"] if "model" in model_ckpt else model_ckpt, strict=False
        )
    else:
        model = VisionTransformerINT8(
            config, args.img_size, zero_head=True, num_classes=5
        )
...
def calib(args, config, model):
    ...
    output_model_path = os.path.join(
        args.calib_output_path, "{}_calib.pth".format(args.model_type)
    )
...
def train(args, config):
    if CALIBRATION:
        ...
        model = VisionTransformerINT8(
            model_config, args.img_size, zero_head=False, num_classes=num_classes
        )
    else:
        num_classes = 5
        model_config = CONFIGS[args.model_type]
    ...
    if args.validate:
        model.eval()
        dataset_train, dataset_val, train_loader, test_loader = build_loader(
            config, args
        )
        quant_utils.configure_model(model, args, calib=False)
        accuracy = valid(args, config, model, test_loader)
    ...
    if args.distill:
        ...
        dis_loss = Knowledge_Distillation_Loss(
    ...
    optimizer = torch.optim.SGD(
        model.parameters(), lr=args.qat_lr, momentum=0.9, weight_decay=args.weight_decay
    )
    ...
    for epoch_i in range(args.num_epochs):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if teacher:
                ...
                loss_t = dis_loss.get_knowledge_distillation_loss(
...
def parse_option():
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Perform validation only"
    )
    parser.add_argument(
        "--throughput", action="store_true", help="Test throughput only"
    )
    parser.add_argument(
        "--posttrain", action="store_true", help="Calibrate post-train .bin"
    )
    parser.add_argument(
        "--num-calib-batch",
        type=int,
    ...
    parser.add_argument(
        "--calib-batchsz", type=int, default=8, help="Batch size when doing calibration"
    )
    parser.add_argument(
        "--calib-output-path",
        type=str,
        default="calib-checkpoint",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of epochs to run QAT fintuning.",
    )
    parser.add_argument(
        "--qat-lr", type=float, default=1e-6, help="learning rate for QAT."
    )
