...
class ViTINT8WeightLoader(object):
    def __init__(
        self, layer_num, img_size, patch_size, weight_dict=None, classifier="token"
    ):
        ...
        if weight_dict is None:
            print(
                "[ERROR][SwinTransformerWeights::__init__] weights should not be empty!"
            )
            exit(-1)
            ...
            if name.split(".")[-1] == "pos_embedding":
                ...
                if th_weight.size(1) != posemb_new_size:
                    print(
                        ...
                        % (th_weight.size(1), posemb_new_size)
                    )
    ...
    def to_int8(self, ths_path="../../../../build/lib/libth_transformer.so"):
        if (
            "transformer.encoder.layer.0.attn.query._input_quantizer._amax"
            not in self.weights
        ):
            raise RuntimeError(
